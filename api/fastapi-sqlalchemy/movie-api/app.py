...
@app.post('/movies')
def post_movies(body: MovieInput, authorization: str = Header(None)):
    token = authorization.replace("Bearer ", "")
    payload = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
    username = payload["sub"]
    with Session(engine) as session:
        user = session.query(User).filter(User.username == username).first()
        movie = Movie(title=body.title, genre=body.genre, owner_id=user.id)
        session.add(movie)
        session.commit()
...
def delete_movies(id: int, authorization: str = Header(None)):
    token = authorization.replace("Bearer ", "")
    payload = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
    username = payload["sub"]
    with Session(engine) as session:
